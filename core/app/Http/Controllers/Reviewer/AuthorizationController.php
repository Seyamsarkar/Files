...
class AuthorizationController extends Controller {
    ...
    public function authorizeForm() {
        ...
        if (!$this->checkCodeValidity($reviewer) && ($type != '2fa') && ($type != 'ban')) {
            $reviewer->ver_code         = verificationCode(6);
            $reviewer->ver_code_send_at = Carbon::now();
            $reviewer->save();
            notify($reviewer, $notifyTemplate, [
                'code' => $reviewer->ver_code,
            ], [$type]);
        }

        return view('reviewer.auth.authorization.' . $type, compact('reviewer', 'pageTitle'));
    }
