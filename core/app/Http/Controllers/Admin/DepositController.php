...
class DepositController extends Controller
{
    ...
    protected function depositData($scope = null, $summery = false)
    {
        ...
        $deposits = $deposits->with(['user', 'gateway'])->where('method_code', '!=', 0)->searchable(['trx', 'user:username'])->dateFilter();
        $request  = request();
    ...
    {
        $general   = gs();
        $deposit   = Deposit::where('id', $id)->with(['user', 'gateway', 'sell', 'sell.product'])->firstOrFail();
